...
def main():
    ...
    common = ["--workers", "1", "--worker-class", "gthread", "--threads", "8", "--timeout", "30", "--log-level", "info"]
    agent_log = open(state / "agent.log", "w")
    gateway_log = open(state / "gateway.log", "w")
    procs = [
        subprocess.Popen([gunicorn, "--bind", f"127.0.0.1:{pa}", *common, "harness_agent:app"], cwd=HERE, env=env, stdout=agent_log, stderr=subprocess.STDOUT),
    ]
    check("agent gunicorn up", wait_port(pa))
    procs.append(
        subprocess.Popen([gunicorn, "--bind", f"127.0.0.1:{pg}", *common, "harness_gateway:app"], cwd=HERE, env=env, stdout=gateway_log, stderr=subprocess.STDOUT)
    )
    check("gateway gunicorn up", wait_port(pg))
    time.sleep(1)
    jwt = JWTManager.key(audience="slurm-web", algorithm="HS256", path=state / "jwt.key")
    ...
    try:
        ...
        ws.send(json.dumps({"type": "subscribe", "checks_cursor": 0, "log": {"stream": "stdout", "offset": 0}}))
        msgs = collect(ws, 2.5, stop_when=lambda m: m["type"] == "gpu")
        t = types(msgs)
        check("subscribe yields checks snapshot", "checks" in t and msgs[t.index("checks")]["cursor"] == 100, str(t))
        check("subscribe yields job summary", "job" in t and msgs[t.index("job")]["active"] is True, str(t))
        check("subscribed ack carries cursors", "subscribed" in t and msgs[t.index("subscribed")]["cursors"]["checks"] == 100)
        logs = [m for m in msgs if m["type"] == "log"]
        check("initial log chunk delivered", logs and logs[0]["chunk"] == "line 1\nline 2\n" and logs[0]["next_offset"] == 14, str(logs[:1]))
        gpus = [m for m in msgs if m["type"] == "gpu"]
        check("gpu telemetry delivered once", len(gpus) == 1 and gpus[0]["summary"]["count"] == 1)
        ...
        logs = [m for m in msgs if m["type"] == "log"]
        check("log append streams incrementally", logs and logs[-1]["chunk"] == "line 3\n" and logs[-1]["offset"] == 14, str(logs[-1:]))
        # Check event append.
        ...
        check("check event streams with seq", evs and evs[-1]["event"]["seq"] == 101 and evs[-1]["event"]["progress"] == 40)
        # Switch to stderr.
        err.write_text("boom\n")
        ws.send(json.dumps({"type": "log", "stream": "stderr", "offset": 0}))
        msgs = collect(ws, 2.0, stop_when=lambda m: m["type"] == "log")
        logs = [m for m in msgs if m["type"] == "log"]
        check("stream switch to stderr", logs and logs[-1]["stream"] == "stderr" and logs[-1]["chunk"] == "boom\n", str(logs[-1:]))
        # Pause / resume.
        ...
        check("paused log channel stays quiet", "log" not in types(msgs), str(types(msgs)))
        ws.send(json.dumps({"type": "resume", "channel": "log"}))
        msgs = collect(ws, 2.0, stop_when=lambda m: m["type"] == "log")
        logs = [m for m in msgs if m["type"] == "log"]
        check("resume flushes buffered bytes at right offset", logs and logs[-1]["chunk"] == "after pause\n" and logs[-1]["offset"] == 5, str(logs[-1:]))
        # Ping / pong.
        ...
        job["time"]["elapsed"] = 9
        (state / "job.json").write_text(json.dumps(job))
        msgs = collect(ws, 4.0, stop_when=lambda m: m["type"] == "job" and not m["active"])
        jobs = [m for m in msgs if m["type"] == "job"]
        check("job completion pushed", jobs and jobs[-1]["active"] is False and jobs[-1]["elapsed"] == 9, str(jobs[-1:]))
        # Heartbeat when idle.
        ...
        check("heartbeat within 10s idle", "heartbeat" in types(msgs) and types(msgs).count("gpu") <= 1, str(types(msgs)))
        ws.close()

        # 2. Resume with cursors: nothing before the cursor is resent.
        ws = Client.connect(agent_url, headers={"Authorization": f"Bearer {token}"})
        collect(ws, 1.0, stop_when=lambda m: m["type"] == "ready")
        ws.send(json.dumps({"type": "subscribe", "channels": ["checks", "log"], "checks_cursor": 101, "log": {"stream": "stdout", "offset": 14}}))
        msgs = collect(ws, 2.5)
        evs = [m for m in msgs if m["type"] == "check"]
        logs = [m for m in msgs if m["type"] == "log"]
        check("resume skips already-seen check events", not evs, str(types(msgs)))
        check("resume continues log from byte offset", logs and logs[0]["offset"] == 14 and logs[0]["chunk"] == "line 3\n", str(logs[:1]))
        check("unsubscribed channels stay silent", "gpu" not in types(msgs) and "job" not in types(msgs))
        ws.close()
        ...
        check("bad header token closes with 4401", int(code) == 4401 and msgs and msgs[0]["type"] == "error", f"code={code} msgs={types(msgs)}")
        ws = Client.connect(agent_url)
        ws.send(json.dumps({"type": "auth", "token": "nope"}))
        collect(ws, 2.0)
        check("bad auth frame closes with 4401", int(ws.close_reason) == 4401, str(ws.close_reason))
        ws = Client.connect(agent_url)
        ws.send(json.dumps({"type": "subscribe"}))
        collect(ws, 2.0)
        check("missing auth frame closes with 4401", int(ws.close_reason) == 4401, str(ws.close_reason))
        ws = Client.connect(agent_url, headers={"Authorization": f"Bearer {token}"})
        collect(ws, 1.0, stop_when=lambda m: m["type"] == "ready")
        ws.send("not json")
        collect(ws, 2.0)
        check("garbage frame closes with 4400", int(ws.close_reason) == 4400, str(ws.close_reason))
        ws = Client.connect(f"ws://127.0.0.1:{pa}/v{V}/job/2/live", headers={"Authorization": f"Bearer {token}"})
        # job 2 is served by the stub too (same job.json), so only user mismatch would 404; skip.
        ws.close()
        ...
        t = types(msgs)
        check("gateway relays ready/checks/job/log/gpu", all(k in t for k in ("ready", "checks", "job", "subscribed", "log", "gpu")), str(t))
        with out.open("a") as f:
            f.write("via gateway\n")
        msgs = collect(ws, 2.5, stop_when=lambda m: m["type"] == "log")
        logs = [m for m in msgs if m["type"] == "log"]
        check("gateway relays incremental log", logs and logs[-1]["chunk"] == "via gateway\n", str(logs[-1:]))
        ws.send(json.dumps({"type": "ping"}))
        msgs = collect(ws, 1.5, stop_when=lambda m: m["type"] == "pong")
        ...
        ws.send(json.dumps({"type": "auth", "token": "nope"}))
        collect(ws, 2.0)
        check("gateway rejects bad token with 4401", int(ws.close_reason) == 4401, str(ws.close_reason))
        ws = Client.connect(f"ws://127.0.0.1:{pg}/api/agents/nope/job/1/live")
        ws.send(json.dumps({"type": "auth", "token": token}))
        collect(ws, 2.0)
        check("gateway unknown cluster closes 4404", int(ws.close_reason) == 4404, str(ws.close_reason))
        ws = Client.connect(gateway_url)
        ws.send(json.dumps({"type": "auth", "token": token}))
        ws.send(json.dumps({"type": "subscribe", "checks_cursor": 0, "log": {"stream": "stdout", "offset": 0}}))
        collect(ws, 1.5, stop_when=lambda m: m["type"] == "subscribed")
        # Kill the agent-side session by closing upstream: simulate by closing browser side and
        ...
        def session(i):
            ...
            c.send(json.dumps({"type": "subscribe", "checks_cursor": 0, "log": {"stream": "stdout", "offset": 0}}))
        ...
        req = urllib.request.Request(f"http://127.0.0.1:{pa}/v{V}/job/1/log?stream=stdout&offset=0", headers={"Authorization": f"Bearer {token}"})
        started = time.monotonic()
        with urllib.request.urlopen(req, timeout=5) as resp:
            body = json.loads(resp.read())
        check("REST log endpoint responsive during 5 live sessions", resp.status == 200 and body["chunk"].startswith("line 1"), f"{time.monotonic()-started:.2f}s")
        for c, _ in sessions:
            c.close()
        time.sleep(1.5)
    except Exception:
        ...
        if failed:
            print("---- agent.log tail"); print(alog[-4000:])
            print("---- gateway.log tail"); print(glog[-4000:])
